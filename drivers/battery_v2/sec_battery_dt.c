		...
		struct sec_battery_info *battery)
{
	...
	if (ret) {
		pr_debug("%s : cisd_max_voltage_thr_step is Empty\n", __func__);
	...
	if (ret)
		pr_debug("%s : dchg_thermal_source is Empty\n", __func__);
	...
	if(pdata->dchg_thermal_source == SEC_BATTERY_THERMAL_SOURCE_CHG_ADC) {
		...
		for(i = 0; i < pdata->dchg_temp_adc_table_size; i++) {
			...
			if (ret)
				pr_debug("%s : DIRECT CHG_Temp_adc_table(adc) is Empty\n",
					__func__);
			...
			if (ret)
				pr_debug("%s : DIRECT CHG_Temp_adc_table(data) is Empty\n",
					__func__);
		}
	}
	...
	if (ret)
		pr_debug("%s : dchg_temp_check_type is Empty\n", __func__);
#endif
	/* wpc thermistor */
	...
	if (ret)
		pr_debug("%s : blkt_temp_check_type is Empty\n", __func__);
	...
	if (ret) {
		pr_debug("%s : tx_gear_vout is Empty\n", __func__);
	...
	if (ret) {
		pr_debug("%s : tx_gear_vout_delay is Empty\n", __func__);
	...
	if (ret) {
		pr_debug("%s: swelling_low_temp_current is Empty, Default value 600mA\n", __func__);
	...
	if (ret) {
		pr_debug("%s: swelling_low_temp_topoff is Empty, Default value 200mA\n", __func__);
	...
	if (ret) {
		pr_debug("%s: swelling_high_temp_current is Empty, Default value 1300mA\n", __func__);
	...
	if (ret) {
		pr_debug("%s: swelling_high_temp_topoff is Empty, Default value 200mA\n", __func__);
	...
	if (ret) {
		pr_debug("%s: swelling_wc_high_temp_current is Empty, Default value 600mA\n", __func__);
	...
	if (ret) {
		pr_debug("%s: swelling_wc_low_temp_current is Empty, Default value 600mA\n", __func__);
	...
	if (battery->swelling_low_temp_4th_ctrl) {
		...
		if (ret) {
			pr_debug("%s: swelling low temp recovery 4th is Empty\n", __func__);
		...
		if (ret) {
			pr_debug("%s: swelling_low_temp_current_4th is Empty, set swelling_low_temp_current value\n", __func__);
	...
	if (battery->swelling_low_temp_5th_ctrl) {
		...
		if (ret) {
			pr_debug("%s: swelling low temp recovery 5th is Empty\n", __func__);
		...
		if (ret) {
			pr_debug("%s: swelling_low_temp_current_5th is Empty, set swelling_low_temp_current value\n", __func__);
	...
	if(battery->swelling_low_temp_3rd_ctrl)
		pr_debug("%s : SWELLING_LOW_TEMP_3rd(%d) SWELLING_LOW_TEMP_RECOVERY_3rd(%d) SWELLING_LOW_CURRENT(%d) SWELLING_LOW_CURRENT_WC(%d)\n",
	...
	if(battery->swelling_low_temp_4th_ctrl)
		pr_debug("%s : SWELLING_LOW_TEMP_4th(%d) SWELLING_LOW_TEMP_RECOVERY_4th(%d) SWELLING_LOW_CURRENT(%d)\n",
			__func__, pdata->swelling_low_temp_block_4th, pdata->swelling_low_temp_recov_4th,
			pdata->swelling_low_temp_current_4th);

	if(battery->swelling_low_temp_5th_ctrl)
		pr_debug("%s : SWELLING_LOW_TEMP_5th(%d) SWELLING_LOW_TEMP_RECOVERY_5th(%d) SWELLING_LOW_CURRENT(%d)\n",
	...
	if (ret) {
		pr_debug("%s: ttf_hv_12v_wireless_charge_current is Empty, Default value 0\n", __func__);
		pdata->ttf_hv_12v_wireless_charge_current =
	...
	if (ret) {
		pr_debug("%s: ttf_dc25_charge_current is Empty, Default value 0\n", __func__);
		pdata->ttf_dc25_charge_current =
			pdata->charging_current[SEC_BATTERY_CABLE_9V_TA].fast_charging_current;
	}
	...
	if (ret) {
		pr_debug("%s: ttf_dc45_charge_current is Empty, Default value 0\n", __func__);
	...
	if (ret) {
		pr_debug("%s: ttf_normal_charge_current is Empty, Default value\n", __func__);
	...
	if (ret) {
		...
		pdata->ttf_pogo_charge_current =
			pdata->charging_current[SEC_BATTERY_CABLE_POGO].input_current_limit;
	}
#endif
#endif
	...
	if (ret) {
		pr_debug("support_dual_battery is empty\n");
	...
	if (pdata->support_dual_battery) {
		...
		if (ret)
			pr_debug("%s: Main limiter name is Empty\n", __func__);

		ret = of_property_read_string(np,
			"battery,sub_limiter_name", (char const **)&pdata->sub_limiter_name);
		if (ret)
			pr_debug("%s: Sub limiter name is Empty\n", __func__);		
	}
	...
	if (!np) {
		...
		pr_debug("%s : main rate:%d, sub rate:%d, recharge marging:%d, "
